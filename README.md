...
1. The current model is not 'symmetric'. By this I mean that the weights for fighter_x and fighter_y features are not equal. This can be seen in the feature importance plot above; for example, age_y is more important than age_x. This mean that the model has some bias. The model is actually favours fighter y winning. This is not due to fighter_y winning more often in the dataset as this was checked to be a 50/50 split between fighter x and fighter y winning. The origin of this bias is still unknown, however a workaround was implemented. The application actually runs the model twice, swapping the order of the fighters input into the model in the second iteration. The model then presents to the user the most confident prediction of the two models, thus ensuring that the same result is given each time, no matter what order the fighter's are input.
